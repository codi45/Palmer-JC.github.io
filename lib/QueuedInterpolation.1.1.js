...
(function (QI) {
    ...
    var MatrixComp = (function () {
         ...
         * This is called on the MatrixComp version of a Bone's Basis (Rest) pose when the library is assigned.
         * @param {QI.MatrixComp} libraryBasis - The Rest value of this bone in the library.
         */
        MatrixComp.prototype.setBasisRatios = function (libraryBasis, report) {
            ...
            if (report) {
                console.log("pelvis translationBasisRatio: " + this.translationBasisRatio.toString());
            }
        };
        /**
         ...
         * @param {QI.MatrixComp} libraryPose - The value of this bone for a pose in the library.
         */
        MatrixComp.prototype.getRelativePose = function (libraryPose, skelDimensionsRatio, report) {
            // when there are diffs to Basis, build relative target
            if (libraryPose.rotationBasisDiff) {
                ...
                var relTrans = libraryPose.translationBasisDiff.add(this.translation);
                //                var relTrans = libraryPose.translationBasisDiff.scale(boneLengthToLibraryRatio).addInPlace(this.translation);
                var retScale = libraryPose.scaleBasisDiff.multiply(this.scaleBasisRatio).addInPlace(this.scale);
                if (report)
                    console.log("libraryPose.translationBasisDiff: " + libraryPose.translationBasisDiff + "ratio " + this.translationBasisRatio + " rest " + this.translation);
        ...
        /**
         * Re-compose the components back into a matrix
         */
        ...
        MatrixComp.prototype.toString = function (boneName, asMatrix) {
            var ret = boneName + ": ";
            if (asMatrix) {
                ...
                for (var i = 0; i < 16; i++) {
                    if (i > 0)
                        ret += ",";
                    ret += temp.m[i].toFixed(4);
                }
            }
            else {
                ret += " Trans- {X:" + this.translation.x.toFixed(4) + "Y:" + this.translation.y.toFixed(4) + "Z:" + this.translation.z.toFixed(4) + "}";
                ret += ", Rot- {X:" + this.rotation.x.toFixed(4) + "Y:" + this.rotation.y.toFixed(4) + "Z:" + this.rotation.z.toFixed(4) + "}";
                if (MatrixComp.needScale(this.scale))
                    ret += ", Scale- {X:" + this.scale.x.toFixed(4) + "Y:" + this.scale.y.toFixed(4) + "Z:" + this.scale.z.toFixed(4) + "}";
            }
            return ret;
        };
        /**
...
/// <reference path="../gif/GifMaker.ts"/>
    ...
    var TimelineControl = (function () {
        ...
        TimelineControl._manualAdvanceAfterRender = function () {
            ...
            if (!TimelineControl._systemPaused || TimelineControl._resumeQueued) {
                ...
                if (TimelineControl._resumeQueued || BABYLON.Tools.Now - TimelineControl._lastRun > TimelineControl.CHANGED_TABS_THRESHOLD) {
                ...
                else if (!TimelineControl._isRealtime) {
                    ...
                    if (TimelineControl.gifMaker) {
                        TimelineControl.gifMaker.addFrame();
                    }
                }
            }
...
// this is relies files from https://github.com/antimatter15/jsgif
var GIFEncoder;
var QI;
(function (QI) {
    var GifMaker = (function () {
        function GifMaker(fileName, engine, frameRate) {
            ...
            this.encoder.setComment("Generated in QueuedInterpolation by GifMaker");
            this.encoder.setSize(this.width, this.height);
            this.encoder.setRepeat(0); // infinite repeats
            this.encoder.setFrameRate(this.frameRate);
            this.encoder.setQuality(1); // highest possible
            this.encoder.setTransparent(this.toRGBNumber(scene.clearColor));
            this.encoder.start();
            console.log(" color " + this.toRGBNumber(scene.clearColor));
            // switch to fixed frame rate of 24
            QI.TimelineControl.change(false, this.frameRate);
            QI.TimelineControl.gifMaker = this;
        }
        /**
         * An aid to make reduce the # of files of a scene by one, the html file.  This will dynamically
         * load the js files required to support GifMaker.
         * @param {string} jsPath- This is the Url directory where GIFEncoder.js, LZWEncoder.js, & NeuQuant.js
         * exist.  May be relative to html file.
         */
        GifMaker.librariesLoad = function (jsPath) {
            if (!GIFEncoder) {
                if (jsPath.lastIndexOf("/") + 1 !== jsPath.length)
                    jsPath += "/";
                var encoderElem = document.createElement("script");
                encoderElem.type = "application/javascript";
                encoderElem.src = jsPath + "GIFEncoder.js";
                document.body.appendChild(encoderElem);
                var LZWElem = document.createElement("script");
                LZWElem.type = "application/javascript";
                LZWElem.src = jsPath + "LZWEncoder.js";
                document.body.appendChild(LZWElem);
                var quantElem = document.createElement("script");
                quantElem.type = "application/javascript";
                quantElem.src = jsPath + "NeuQuant.js";
                document.body.appendChild(quantElem);
            }
        };
        GifMaker.prototype.addFrame = function () {
            this.encoder.addFrame(this.DumpFramebuffer(), true);
        };
        GifMaker.prototype.finish = function () {
            this.encoder.finish();
            this.encoder.download(this.fileName);
            QI.TimelineControl.pauseSystem();
        };
        //8355711
        GifMaker.prototype.toRGBNumber = function (color) {
         ...
         */
        GifMaker.prototype.DumpFramebuffer = function () {
...
(function (QI) {
    ...
    MotionEvent.SINE_INOUT_PACE = new QI.SinePace(QI.Pace.MODE_INOUT);
    // ========================================== Enums  =========================================
...
(function (QI) {
    var Bone = (function (_super) {
        ...
        Bone.prototype.assignPoseLibrary = function (library) {
            var libraryBasis = library.poses["basis"].targets[this.name];
            this._restComp.setBasisRatios(libraryBasis, this.name === "pelvisX");
            this._boneLengthToLibraryRatio = this.length / library.boneLengths[this.name];
        };
         ...
         */
        Bone.prototype._assignPose = function (libraryPose, immediately) {
            var skelDimensionsRatio = this.getSkeleton()._skelDimensionsRatio;
            this._rel_target = this._restComp.getRelativePose(libraryPose, skelDimensionsRatio, this.name === "pelvisX");
            //            console.log("\t" + this._rel_target.toString(this.name));
...
(function (QI) {
    ...
    var Mesh = (function (_super) {
        ...
        /**
         * Queue a function to run on a queue
         * @param {function} func - The function to run.
         * @param {string} groupName - The processor / queue to have the function.
         */
        Mesh.prototype.queueFunction = function (func, groupName) {
            if (groupName === void 0) { groupName = QI.PovProcessor.POV_GROUP_NAME; }
            var stall = new QI.Stall(1, groupName); // needed to make a return value
            var events = [func, stall];
        ...
        Object.defineProperty(Mesh.prototype, "visibilityAndKids", {
            /**
             * A setter of the visibility which also sets the child meshes.  Being a setter
             * allows it to be part of a property setter.
             */
            set: function (value) {
                var children = this.getChildMeshes();
                for (var i = 0, len = children.length; i < len; i++) {
...
(function (QI) {
    var Hair = (function (_super) {
         ...
         * @param {Node} parent - The parent of this mesh, if it has one
         */
        function Hair(name, scene, parent) {
            if (parent === void 0) { parent = null; }
            var _this = _super.call(this, name, scene, parent) || this;
            _this._parentVertDelta = 0;
            // from https://stackoverflow.com/questions/521295/seeding-the-random-number-generator-in-javascript
            _this._seed = 0;
            if (parent && parent instanceof BABYLON.Mesh)
                _this.assignParentDynamically(parent);
            return _this;
        }
         ...
         */
        Hair.prototype.assemble = function (cleanGeo) {
            if (cleanGeo === void 0) { cleanGeo = true; }
        ...
        Hair.prototype.assignParentDynamically = function (parent, isHairOnTop) {
            if (isHairOnTop === void 0) { isHairOnTop = false; }
            this.parent = parent;
         ...
         */
        Hair.prototype._toAbsolutePositions = function () {
            var pdx = 0; // index used for writing into positions
            this._nPosElements = 0;
            var absPositions = new Array(this.rootRelativePositions.length);
        ...
        Hair.prototype._setCornerOffset = function () {
            // allow the value to be in the export, but not currently done
            if (this.cornerOffset)
                return;
            var val;
            // assign all in meters
            switch (this.strandWidth) {
                case "THIN_WIDTH":
                    val = 0.001;
                    break;
                case "THICK_WIDTH":
            ...
            switch (this.unit) {
                case "METER":
                    this.cornerOffset = val;
                    break;
                case "INCH":
                    this.cornerOffset = val * 39.3701;
                    break;
                case "CENTIMETER":
                    this.cornerOffset = val * 100;
                    break;
                case "DECIMETER":
                default:
                    this.cornerOffset = val * 10;
                    break;
            }
        };
        /**
         ...
         * strands.
         */
        Hair.prototype._assignWeights = function (absPositions) {
         ...
         */
        Hair.prototype.assignWeightsAsColor = function () {
            ...
            if (!this.material) {
                ...
                if (this.parent instanceof BABYLON.Mesh) {
                    ...
                    if (parent && parent.material && parent.material instanceof BABYLON.StandardMaterial)
                        material.maxSimultaneousLights = parent.material.maxSimultaneousLights;
                }
                this.material = material;
            }
            var emissiveColor = this.color.scale(this.emissiveColorScaling);
            if (emissiveColor.r > 0.5)
                emissiveColor.r = 0.5;
            if (emissiveColor.g > 0.5)
                emissiveColor.g = 0.5;
            if (emissiveColor.b > 0.5)
                emissiveColor.b = 0.5;
            this.material.emissiveColor = emissiveColor;
        };
...
(function (QI) {
    ...
    var AudioRecorder = (function () {
         ...
         */
        AudioRecorder.prototype.recordStart = function (durationMS, doneCallback, dynamicStartCallBack) {
    ...
    AudioRecorder._startThreshold = 0.05;
...
(function (QI) {
    ...
    var Expression = (function () {
        ...
        Object.defineProperty(Expression.prototype, "regularNameFor", {
            get: function () {
                return (!this.isViseme) ? this.name.replace("_TALK", "") : null;
            },
            enumerable: true,
            configurable: true
        });
        Object.defineProperty(Expression.prototype, "speechCapableNameFor", {
            /**
             * @returns the string name of the version of this expression which should be used for speech.
             */
            get: function () {
                if (this.isSpeechCapable || this.name === "NONE")
                    return this.name;
                return (!this.isViseme) ? this.name + "_TALK" : null;
            },
            enumerable: true,
            configurable: true
        });
        /**
         * Returns the name of the expression which is the talking version
         * @param {string} name - The name of the expression to get the talking version of.
         */
        Expression.convertForSpeech = function (name) {
            if (name.indexOf("_TALK") !== -1 || name === "NONE")
                return name;
            return name + "_TALK";
        };
        Expression.prototype.softenMouthTargets = function (mouthMultiplier) {
            if (mouthMultiplier === void 0) { mouthMultiplier = 0.5; }
            var name = this.name + "_TALK";
            var endStateNames = [];
            var endStateRatios = [];
            var mirrorAxes = null;
            var mirrorReqd = false;
            for (var i = 0, nStates = this.endStateNames.length; i < nStates; i++) {
                if (this.endStateNames[i].indexOf("MOUTH_") === -1 && this.endStateNames[i].indexOf("TONGUE_") === -1) {
                ...
                else if (this.endStateNames[i].indexOf("MOUTH_") > -1 && this.endStateNames[i] !== "MOUTH_OPEN") {
                    endStateNames.push(this.endStateNames[i]);
                    endStateRatios.push(this.endStateRatios[i] * mouthMultiplier);
                    mirrorReqd = mirrorReqd || this.endStateRatios[i] < 0;
                }
            }
    ...
    Expression.DISGUSTED = new Expression("DISGUSTED", false, true, true, ["CHEEKS_HIGH", "CHEEKS_PUMP", "CHEEKS_SUCK", "EYEBROWS_ANGRY", "EYEBROWS_RAISED_LEFT", "EYELIDS_SQUINT", "WINK_LEFT", "NOSE_FLARE", "MOUTH_CORNERS_DOWN", "MOUTH_CORNERS_UP", "MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP"], [-1.25, 0.65, 0.05, 0.55, 1, 0.45, -0.1, 1.35, 1.3, -0.1, 0.2, -1], "Y-----Y--Y-Y", Expression.STOCK);
    Expression.GRUMPY = new Expression("GRUMPY", false, false, true, ["CHEEKS_HIGH", "CHEEKS_PUMP", "EYEBROWS_ANGRY", "EYEBROWS_RAISED_LEFT", "NOSE_FLARE", "MOUTH_CORNERS_DOWN", "MOUTH_CORNERS_UP", "MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_OPEN"], [-2, 1.15, 2, -0.4, -0.35, -1.5, -0.35, 0.65, 0.2, -0.25, -0.45], "Y--YYYY--YZ", Expression.STOCK);
    Expression.HAPPY = new Expression("HAPPY", true, true, true, ["CHEEKS_HIGH", "CHEEKS_PUMP", "CHEEKS_SUCK", "EYEBROWS_ANGRY", "EYEBROWS_RAISED_LEFT", "EYEBROWS_RAISED_RIGHT", "EYELIDS_SQUINT", "NOSE_FLARE", "MOUTH_CORNERS_DOWN", "MOUTH_CORNERS_UP", "MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_OPEN", "MOUTH_PUCKER", "MOUTH_WIDE", "SYMMETRY_CHIN_LEFT", "SYMMETRY_LEFT_UP", "SYMMETRY_RIGHT_UP"], [-0.85, 1.05, 0.5, -1.2, 0.55, 0.4, 0.1, 0.3, -0.55, 0.55, 0.55, -1.05, -0.25, 0.1, -0.4, 0.75, -0.1, 0.1, 0.1], "Y--Y----Y--YY-X-X--", Expression.STOCK);
    Expression.LAUGH = new Expression("LAUGH", true, true, true, ["CHEEKS_HIGH", "CHEEKS_PUMP", "CHEEKS_SUCK", "EYEBROWS_ANGRY", "EYEBROWS_RAISED_LEFT", "EYEBROWS_RAISED_RIGHT", "EYELIDS_SQUINT", "NOSE_FLARE", "MOUTH_CORNERS_UP", "MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_OPEN", "MOUTH_PUCKER", "MOUTH_WIDE", "SYMMETRY_RIGHT_UP"], [-0.45, 0.9, 0.45, -1.2, 0.55, 0.4, 0.8, 0.3, 0.35, 2, -0.95, -0.25, 0.4, -0.4, 1.5, 0.1], "Y--Y------ZY-X--", Expression.STOCK);
    ...
    Expression.STRUGGLING = new Expression("STRUGGLING", false, false, false, ["CHEEKS_HIGH", "CHEEKS_PUMP", "EYEBROWS_ANGRY", "EYEBROWS_RAISED_LEFT", "EYELIDS_SQUINT", "WINK_LEFT", "WINK_RIGHT", "NOSE_FLARE", "MOUTH_CORNERS_DOWN", "MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_WIDE", "SYMMETRY_LEFT_UP", "SYMMETRY_RIGHT_UP"], [-0.85, 0.4, 1, 0.6, 1, 0.25, 0.35, 1, 0.5, 0.35, -0.7, 0.6, 0.05, -0.4, 0.55], "Y---------Y--Y-", Expression.STOCK);
    Expression.ANGRY_TALK = Expression.ANGRY.softenMouthTargets();
    Expression.CRYING_TALK = Expression.CRYING.softenMouthTargets();
    Expression.DISGUSTED_TALK = Expression.DISGUSTED.softenMouthTargets();
    Expression.GRUMPY_TALK = Expression.GRUMPY.softenMouthTargets();
    Expression.HAPPY_TALK = Expression.HAPPY.softenMouthTargets();
    Expression.LAUGH_TALK = Expression.LAUGH.softenMouthTargets();
    Expression.SAD_TALK = Expression.SAD.softenMouthTargets();
    Expression.SCARED_TALK = Expression.SCARED.softenMouthTargets();
    Expression.SKEPTICAL_TALK = Expression.SKEPTICAL.softenMouthTargets();
    Expression.STRUGGLING_TALK = Expression.STRUGGLING.softenMouthTargets();
    ...
    Expression.VISEME_DICT = {
        ...
        "F-V": new Expression("F-V", true, true, false, ["MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_WIDE"], [.6, 0.2, -0.5, 0.45], "--Y-", Expression.VISEME),
        "IY": new Expression("IY", true, true, false, ["MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_OPEN"], [0.05, 0.1, 0.4, 0.1], null, Expression.VISEME),
        "L": new Expression("L", true, true, false, ["MOUTH_LIPS_LOWER_OUT", "MOUTH_LIPS_LOWER_UP", "MOUTH_LIPS_UPPER_UP", "MOUTH_OPEN", "MOUTH_PUCKER", "MOUTH_WIDE", "TONGUE_RAISED", "TONGUE_STUCK_OUT"], [0.35, -0.7, -0.2, 0.2, -0.05, 0.45, 0.2, 0.2], "-YY-X---", Expression.VISEME),
    ...
    var Automaton = (function (_super) {
         ...
         * @param {boolean} talkingToo - When true, also add speech friendly versions of the expressions.
         */
        Automaton.prototype.addStockExpressions = function (whichOnes, talkingToo) {
            if (whichOnes === void 0) { whichOnes = ""; }
            if (talkingToo === void 0) { talkingToo = false; }
            var all = whichOnes.length === 0;
            if (all || whichOnes.indexOf("ANGRY") !== -1) {
                this.addExpression(Expression.ANGRY);
                if (talkingToo)
                    this.addExpression(Expression.ANGRY_TALK);
            }
            if (all || whichOnes.indexOf("CRYING") !== -1) {
                this.addExpression(Expression.CRYING);
                if (talkingToo)
                    this.addExpression(Expression.CRYING_TALK);
            }
            if (all || whichOnes.indexOf("DISGUSTED") !== -1) {
                this.addExpression(Expression.DISGUSTED);
                if (talkingToo)
                    this.addExpression(Expression.DISGUSTED_TALK);
            }
            if (all || whichOnes.indexOf("GRUMPY") !== -1) {
                this.addExpression(Expression.GRUMPY);
                if (talkingToo)
                    this.addExpression(Expression.GRUMPY_TALK);
            }
            if (all || whichOnes.indexOf("HAPPY") !== -1) {
                this.addExpression(Expression.HAPPY);
                if (talkingToo)
                    this.addExpression(Expression.HAPPY_TALK);
            }
            if (all || whichOnes.indexOf("LAUGH") !== -1) {
                this.addExpression(Expression.LAUGH);
                if (talkingToo)
                    this.addExpression(Expression.LAUGH_TALK);
            }
...
(function (QI) {
    var ZoomOutTransition = (function () {
         ...
         */
        ZoomOutTransition.prototype.initiate = function (meshes, overriddenMillis, overriddenSound) {
            var camera = meshes[0].getScene().activeCamera;
            if (!camera)
                camera = meshes[0].getScene().activeCameras[0];
    ...
    ZoomOutTransition.NAME = "FLASHBACK";
    ZoomOutTransition._DEFAULT_MILLIS = 5000;
    QI.ZoomOutTransition = ZoomOutTransition;
    // code to run on module load, registering of the transition
    QI.SceneTransition.EFFECTS[ZoomOutTransition.NAME] = new ZoomOutTransition();
})(QI || (QI = {}));
